...
##Confidence Thresholds of Each model (Baruah et al.)
...
resultsDynamic = open("results-dynamic.txt","a")
...
def dynamicCascade(img, correctClass,curTime):
    val = "Incorrect"
    fullDyanmicTime = curTime
    validation = labels[correctClass][1]
    global statsTime
    global evaluationNum
    global countDynamic

    dynamicClass, dynamicConfidence, dynamicTime = classifierD(img,confThreshold_152)
    fullDyanmicTime+=dynamicTime
    if(dynamicClass==validation):
        evaluationNum+=1
        global DynamicCorrect
        DynamicCorrect+=1
        val="Correct"
    statsTime+=fullDyanmicTime
    resultsDynamic.write(dynamicClass+' '+str(dynamicConfidence)[:5]+' '+str(dynamicTime)[:5]+'ms\n'+str(fullDyanmicTime)[:5]+'ms\n'+val+'\n\n')
    countDynamic+=1
...
def idkCascade(img, correctClass):
    ...
    ##0.3 (Arbitrary)
    if(AConfidence<0.3):
        resultsDynamic.write(AClass+' '+str(AConfidence)[:5]+' '+str(ATime)[:5]+'ms\n')
        dynamicCascade(img,correctClass,totalTime)
        return
...
stats.write("Average Time: "+str(statsTime/10000)[:5]+"ms"+
            ...
            "\n\nClassifier B Count: "+str(countC)+"\nAccuracy: "+str((Ccorrect/countC)*100)[:5]+"%"+
            "\n\nClassifier D Count: "+str(countD)+"\nAccuracy: "+str((Dcorrect/countD)*100)[:5]+"%"+
            "\n\nDynamic Classifer Count: "+str(countDynamic)+"\nAccuracy: "+str((DynamicCorrect/countDynamic)*100)[:5]+"%")
